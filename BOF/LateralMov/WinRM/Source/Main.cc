...
auto WSManShellCompletionFunction(
    ...
    if (WsmanError && WsmanError->code != 0) {
        if (WsmanError->code == ERROR_OPERATION_ABORTED) {
            // Operation was aborted, no need to print error
        } else {
            BeaconPrintf(
                CALLBACK_ERROR,
...
void go(char* args, int argc) {
    ...
    // CORREÇÃO: Usar autenticação atual se não tiver credenciais explícitas
    if ( WsUserName && *WsUserName ) {
        ...
        // Usar autenticação atual (Kerberos/NTLM do token atual)
